@extends('gblix.master.layout')

@section('content')
    <div class="container">
        <h1 class="mt-5">Lorem Ipsum</h1>
        <p class="lead">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc euismod scelerisque lacus, nec porttitor elit interdum eget. Integer arcu diam, scelerisque eget dolor ac, dapibus bibendum eros. In auctor malesuada semper.
            Donec lacinia orci eu accumsan bibendum. Praesent ac dignissim augue, a ultrices dolor. In hac habitasse platea dictumst. Praesent ut tellus bibendum, convallis metus ut, auctor erat. Donec cursus diam in nunc rhoncus feugiat.
        </p>

    </div>
@endsection
